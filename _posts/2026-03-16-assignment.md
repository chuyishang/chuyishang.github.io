...
toc:
  ...
  - name: "4. Sharding"
    subsections:
      - name: "4.1 Running Sharding Tests"
      - name: "4.2 Mesh + Mode Validation"
      - name: "4.3 Sharded Batch Pipeline"
      - name: "4.4 Sharded Model + Optimizer Initialization"
      - name: "4.5 Physical Shard Validation Helper"
...
---
{% include figure.liquid loading="eager" path="assets/img/jax-lm/jax-lm_assignment.jpg" class="img-fluid rounded z-depth-1" alt="Pasted image 20260318102536.png" width="416" zoomable=true %}
...
For the specific sharding specs we want to do for each mode, you can refer to the [Implementing Sharding](https://www.chuyishang.com/blog/2026/jax-lm/#implementing-distributed-training) section of the original blog post.
...
- `adapters.get_expected_batch_sharding_spec(mode)`
- `adapters.get_sharded_batch(...)`

`adapters.get_expected_batch_sharding_spec(mode)` already forwards to your mode config via:
- `model.get_batch_sharding_for_mode(mode)`
...
- sharded mode (`mesh` + sharding config), and
- no-sharding mode (`mesh=None`, `sharding_config=None`).
