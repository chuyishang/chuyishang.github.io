---
layout: distill
title: "JAX-LM: Language Modelling and Distributed Training in JAX"
...
We draw heavily upon 2 great resources. The first is Assignment 1 from Stanford's [CS336 course](https://cs336.stanford.edu/), which goes over how to implement language models from scratch in PyTorch. The second resource is the [How to Scale Your Model](https://jax-ml.github.io/scaling-book/) textbook, which explains the theory behind parallelism methods and distributed training.

The goal of this blog is to connect these two perspectives by:
1. **Implementing a language model from scratch in JAX** instead of PyTorch
2. Extending this implementation with **distributed training in both JAX and PyTorch**
3. Providing a simple, customizable codebase for **empirically validating theoretical compute rooflines**.

We also **provide an assignment with test cases** so you can implement all of this in JAX yourself!

>The associated code can be found at [https://github.com/chuyishang/jax-lm/](https://github.com/chuyishang/jax-lm/) and the assignment can be found on the [`assignment` branch](https://github.com/chuyishang/jax-lm/tree/assignment) of the same repo.
{: .block-tip }
...
This post is designed to be read front-to-back, but feel free to jump to whatever is most relevant to you. If you're already comfortable with JAX and NNX, you can go straight to [Implementing Our Model](https://www.chuyishang.com/blog/2026/jax-lm/#implementing-our-model). If you're familiar with distributed training concepts (DP, FSDP, TP) and just want to see the JAX implementation, skip ahead to [Implementing Distributed Training](https://www.chuyishang.com/blog/2026/jax-lm/#implementing-distributed-training) and the [code implementation](https://github.com/chuyishang/jax-lm/tree/main/jax_impl/distributed).
...
2. We provide code implementations of a simple, non-distributed language model in both JAX and PyTorch. You can compare these two implementations side-by-side to get a feel of the differences. These can be found here:
  - [JAX Implementation](https://github.com/chuyishang/jax-lm/tree/main/jax_impl)
  - [PyTorch Implementation](https://github.com/chuyishang/jax-lm/tree/main/pytorch_impl)
3. We extend both the JAX and PyTorch implementations to support distributed training with DP, FSDP, TP, and combined FSDP+TP. These can be found here:
  - [JAX Distributed Implementation](https://github.com/chuyishang/jax-lm/tree/main/jax_impl/distributed)
  - [PyTorch Distributed Implementation](https://github.com/chuyishang/jax-lm/tree/main/pytorch_impl/distributed)
...
2. Secondly, JAX provides a set of very powerful **composable function transformations**. The core idea is that we can write a plain Python or NumPy function and transform it into a more powerful function. Some common transforms are as follows:
    1. [`jax.grad`](#jax-grad), which differentiates a function
    2. [`jax.jit`](#jax-jit), which compiles a function via XLA for fast execution
    3. [`jax.vmap`](#jax-vmap), which vectorizes a function over some batch dimension automatically
    4. [`jax.pmap`](#jax-pmap), which is similar to `jax.vmap` but parallelizes a function across devices
    These functions can be composed arbitrarily (such as `jit(vmap(grad(f)))`) and can often provide huge speedups in practice.

3. Lastly, JAX uses the [**XLA Compiler**](https://openxla.org/xla) as its backend, which optimizes across the entire computation graph instead of operator by operator as PyTorch does. This allows XLA to perform aggressive fusion and optimizations that span multiple operations and even devices, which can unlock large performance gains.

Putting this all together, the use of pure, composable functions allows the JAX compiler to trace entire functional units into an intermediate representation, called the [jaxpr](https://docs.jax.dev/en/latest/jaxpr.html), instead of executing each line in the Python interpreter step-by-step. That's where the speedup comes from.
...
def f(x):
    ...
    return -x + count
	 ...
	 return lax.cond(
		 x > 0, # condition
		lambda x: x + count,  # true branch 
		lambda x: -x + count, # false branch 
		x # argument passed to both branches
	 )
{% endhighlight %}
...
Let's take a look at another example before moving on. For those following along on the assignment, note that the other building blocks of our model have minor syntax differences compared to PyTorch, but follow the same ideas discussed here. 
...
On the other hand, gradients are treated as explicit values in JAX. To get our `grad` object, we use `nnx.value_and_grad`, which is a [transformation](#jax-grad) that turns the `loss_fn` into a function that also returns gradients. Since JAX transforms act as higher order functions, we need to define a loss function and pass it in as input to `nnx.value_and_grad` instead of just calling `loss.backward()` like we do in PyTorch.
...
$$
\text{In}[B, D] \cdot W_{1}[D, F] \cdot W_{2}[F, D] \to \text{Out}[B, D]
$$
...
$$
\text{In}[B_X, D] \cdot W_{1}[D, F] \cdot W_{2}[F,D] \rightarrow \text{Out}[B_X, D]
$$
...
$$
\text{In}[B_X, D] \cdot W_{1}[D_{X}, F] \cdot W_{2}[F,D_{X}] \rightarrow \text{Out}[B_X, D]
$$
...
Now, you can try applying your knowledge with our [assignment](https://github.com/chuyishang/jax-lm/tree/assignment) or refer to our [reference implementations](https://github.com/chuyishang/jax-lm/).
